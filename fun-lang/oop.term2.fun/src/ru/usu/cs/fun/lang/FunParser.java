...
//import ru.usu.cs.fun.front.Strings;
...
public class FunParser extends AbstractLL1Parser {
	...
	public FunParser(FunScope aScope) {
		...
		add("Term", new String[] {"int","float","string", "name"}, new String[] { "Atom", "Tail" }, new ParseAction() {
			public Terms execute(Object[] items) {
		...
		// Term ::= Atom Tail // return Atom.apply(Tail);
		add("Term", new String[] {"name"}, new String[] { "Atom", "Tail" }, new ParseAction() {
			public Terms execute(Object[] items) {
				Term atom = (Term)items[0];
				return new Terms(atom, (Terms) items[1]);
			}
		...
		add("Atom", new String[] { "int" }, new ParseAction() {
			public AllNumber execute(Object[] items) {
				Object value = ((Lexeme)items[0]).getValue();
				return new AllNumber((Integer)value);				
			}
		});
		...
		add("Atom", new String[] { "float" }, new ParseAction() {
			public AllNumber execute(Object[] items) {
				Object value = ((Lexeme)items[0]).getValue();
				return new AllNumber((double)value);				
			}
		});
		add("Atom", new String[] { "string" }, new ParseAction() {
			public MyString execute(Object[] items) {
				Object value = ((Lexeme)items[0]).getValue();
